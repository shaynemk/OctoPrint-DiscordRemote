...
class Command:
    ...
    def status(self):
        ...
        if self.plugin.get_settings().get(['show_local_ip'], merged=True):
            ip_addr = self.plugin.get_settings().get(['hostname'], merged=True) if self.plugin.get_settings().get(['use_hostname_only'], merged=True) else self.plugin.get_ip_address()
        ...
        if self.plugin.get_settings().get(['show_external_ip'], merged=True):
            builder.add_field(title='External IP', text=self.plugin.get_settings().get(['hostname']) if self.plugin.get_settings().get(['use_hostname']) else self.plugin.get_external_ip_address(), inline=True)
